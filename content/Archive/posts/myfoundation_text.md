...
tags: my_foundations
excerpt: >-
  Custom Foundation Model용 라이브러리 중 Text 관련 라이브러리, 그중에서도 LLM 모델 API를 구축한 과정에 대해
  공유드립니다.
...
rag_model = RAG(llm_model=openai_llm, embedding_model=openai_embedding)

# Save the document as vector database
rag_model.fit(documents=documents, db_path=db_path)
